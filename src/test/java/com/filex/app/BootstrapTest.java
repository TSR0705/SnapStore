 ...
 *
 * <p>Phase 1A: Validates that the bootstrap can execute without throwing exceptions and produces a
 * valid {@link AppContext}.
 *
...
class BootstrapTest {
  ...
  void testBootstrapCreatesRequiredDirectories() {
    ...
    assertTrue(context.config().appHome().toFile().exists(), "App home directory should exist");
    assertTrue(context.config().logsDir().toFile().exists(), "Logs directory should exist");
    assertTrue(context.config().dataDir().toFile().exists(), "Data directory should exist");
    assertTrue(context.config().configDir().toFile().exists(), "Config directory should exist");
