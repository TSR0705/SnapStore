...
class IncidentTest {
  ...
  void testIncidentImmutability() {
    ...
    assertThrows(UnsupportedOperationException.class, () -> detectionIds.add("new-detection"));

    Map<String, Object> evidence = incident.getEvidenceSummary();
    assertThrows(UnsupportedOperationException.class, () -> evidence.put("key", "value"));
  }
  ...
  void testToBuilder() {
    ...
    Incident updated =
        original.toBuilder().severity(IncidentSeverity.HIGH).detectionCount(2).build();
  ...
  void testRequiredFields() {
    assertThrows(NullPointerException.class, () -> Incident.builder().build());

    assertThrows(NullPointerException.class, () -> Incident.builder().incidentId("id").build());
  }
}
