...
/**
 * Comprehensive audit test for Phase 1H investigation and replay systems. Tests forensic integrity,
 * replay determinism, and query scalability.
 */
class Phase1HComprehensiveAuditTest {
  ...
  void testReplayDeterminismWithSameMillisecondEvents() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      assertEquals(
          replay1.events().get(i).getSequenceNumber(), replay2.events().get(i).getSequenceNumber());
      assertEquals(
          replay2.events().get(i).getSequenceNumber(), replay3.events().get(i).getSequenceNumber());
    }
  }
  ...
  void testComplexFilterCombinations() throws Exception {
    ...
    createTestIncidentWithDetails(
        "INC-003", "LOW", "LOW", "RESOLVED", now.minus(1, ChronoUnit.DAYS), "CORR-002");
    createTestIncidentWithDetails("INC-004", "HIGH", "HIGH", "OPEN", now, "CORR-002");

    // Test time range + status filter
    InvestigationCriteria criteria1 =
    ...
    // Test correlation + severity filter (note: current implementation doesn't support
    // multi-filter)
    InvestigationCriteria criteria2 =
        InvestigationCriteria.builder().correlationId("CORR-001").build();

    InvestigationResult<IncidentSummary> result2 =
        investigationService.findIncidents(criteria2, 0, 10);
  ...
  void testCorrelationExplorationBounded() throws Exception {
    ...
    for (int i = 0; i < 50; i++) {
      createTestIncidentWithDetails(
          "INC-" + i, "HIGH", "HIGH", "OPEN", Instant.now(), correlationId);
    }
  ...
  void testDeepPaginationStability() throws Exception {
    ...
    InvestigationResult<IncidentSummary> page0 =
        investigationService.findIncidents(criteria, 0, 10);
    InvestigationResult<IncidentSummary> page5 =
        investigationService.findIncidents(criteria, 5, 10);
    InvestigationResult<IncidentSummary> page9 =
        investigationService.findIncidents(criteria, 9, 10);
  ...
  void testPaginationBeyondResults() throws Exception {
    ...
    InvestigationResult<IncidentSummary> result =
        investigationService.findIncidents(criteria, 10, 10);
  ...
  void testLongRangeTimelineTraversal() throws Exception {
    ...
    for (int i = 1; i < window.events().size(); i++) {
      assertTrue(
          window.events().get(i).getTimestamp().isAfter(window.events().get(i - 1).getTimestamp())
              || window
                  .events()
                  .get(i)
                  .getTimestamp()
  ...
  void testConcurrentQueryOperations() throws Exception {
    ...
    for (int i = 0; i < 20; i++) {
      final int page = i % 5;
      futures.add(executor.submit(() -> investigationService.findIncidents(criteria, page, 10)));
    }
  ...
  private String createTestIncident(String incidentId) throws Exception {
    return createTestIncidentWithDetails(incidentId, "HIGH", "HIGH", "OPEN", Instant.now(), null);
  }

  private String createTestIncidentWithDetails(
      String incidentId,
      String severity,
      String confidence,
      String status,
