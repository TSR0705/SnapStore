...
class ReplayNavigationServiceTest {
  ...
  void testReplayCorrelatedEvents() throws Exception {
    ...
    createCorrelatedTimelineEvent(
        incident2, "INCIDENT_CREATED", now.plus(1, ChronoUnit.MINUTES), 1, correlationId);
    createCorrelatedTimelineEvent(
        incident1, "INCIDENT_UPDATED", now.plus(2, ChronoUnit.MINUTES), 1, correlationId);

    ReplayNavigationService.ReplayWindow window =
        replayService.replayCorrelatedEvents(correlationId);
  ...
  void testFindNextCheckpoint() throws Exception {
    ...
    assertTrue(window.events().stream().allMatch(e -> e.getTimestamp().isAfter(checkpoint)));
  }
  ...
  private void createCorrelatedTimelineEvent(
      String incidentId, String eventType, Instant timestamp, long sequence, String correlationId)
      throws Exception {
    ForensicTimelineEntity timeline =
        ForensicTimelineEntity.builder()
            .timelineId(UUID.randomUUID().toString())
