...
class PersistenceFixesValidationTest {
    ...
    try (Statement stmt = conn.createStatement()) {
      SQLException exception =
          assertThrows(
              ...
              () -> {
                stmt.execute(
                    "DELETE FROM incidents WHERE incident_id = '" + incident.getIncidentId() + "'");
              });
      assertTrue(
  ...
  void testFix6_LRUCacheEvictsOldestEntries() {
    ...
    java.util.Map<String, String> lruMap =
  ...
  void testMigrationsV007AndV008Applied() throws Exception {
    ...
    try (Statement stmt = conn.createStatement();
        ResultSet rs =
            stmt.executeQuery(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='incident_evidence'")) {
      assertTrue(rs.next());
    ...
    try (Statement stmt = conn.createStatement();
        ResultSet rs =
            stmt.executeQuery(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_timeline_unique_seq'")) {
  ...
  void testAllMigrationsExecuted() throws Exception {
    ...
    try (Statement stmt = conn.createStatement();
        ResultSet rs =
            stmt.executeQuery("SELECT version, description FROM schema_version ORDER BY version")) {
