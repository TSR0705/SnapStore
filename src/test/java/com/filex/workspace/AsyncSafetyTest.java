...
class AsyncSafetyTest {
  ...
  void testStaleIncidentQueryDiscarded() throws Exception {
    ...
    workspaceService.findIncidentByIdAsync("INC-001", result -> {}, error -> {});

    workspaceService.findIncidentByIdAsync("INC-002", result -> {}, error -> {});
    ...
    assertTrue(
        metrics.asyncQueriesDispatched() >= initialDispatched + 2,
  ...
  void testNavigationHistoryWorks() {
    ...
    assertEquals(
        NavigationContext.INCIDENT_DETAIL, workspaceService.currentState().navigationContext());
    ...
    assertEquals(
        NavigationContext.EVIDENCE_EXPLORATION,
  ...
  void testConcurrentQuerySafety() throws Exception {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      workspaceService.findIncidentByIdAsync(incidentId, result -> {}, error -> {});
    }
    ...
    metrics = workspaceService.getMetrics();
    assertEquals(
        initialDispatched + 10,
        metrics.asyncQueriesDispatched(),
