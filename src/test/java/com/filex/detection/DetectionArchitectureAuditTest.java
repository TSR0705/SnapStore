 ...
 *
 * <p>Validates: - Clean architecture boundaries - No SQL in detection engine - Proper immutability
 * - Rule isolation - Event hierarchy correctness
 */
class DetectionArchitectureAuditTest {
  ...
  void testDetectionEngineHasNoMonitoringEngineDependency() {
    ...
    for (Field field : fields) {
      String typeName = field.getType().getName();
      assertFalse(
          typeName.contains("MonitoringEngine"),
  ...
  void testDetectionEventIsImmutable() {
    assertTrue(
        Modifier.isAbstract(DetectionEvent.class.getModifiers()),
  ...
  private void assertEventClassImmutable(Class<?> eventClass) {
    assertTrue(
        Modifier.isFinal(eventClass.getModifiers()),
  ...
  void testAllDetectionEventsExtendAppEvent() {
    assertTrue(
        AppEvent.class.isAssignableFrom(DetectionEvent.class),
        "DetectionEvent should extend AppEvent");
    assertTrue(
        DetectionEvent.class.isAssignableFrom(MassDeletionDetectedEvent.class),
        "MassDeletionDetectedEvent should extend DetectionEvent");
    assertTrue(
        DetectionEvent.class.isAssignableFrom(RapidModificationDetectedEvent.class),
        "RapidModificationDetectedEvent should extend DetectionEvent");
    assertTrue(
        DetectionEvent.class.isAssignableFrom(SuspiciousRenameDetectedEvent.class),
        "SuspiciousRenameDetectedEvent should extend DetectionEvent");
    assertTrue(
        DetectionEvent.class.isAssignableFrom(HiddenFileDetectedEvent.class),
        "HiddenFileDetectedEvent should extend DetectionEvent");
    assertTrue(
        DetectionEvent.class.isAssignableFrom(SensitiveDirectoryAccessDetectedEvent.class),
  ...
  void testDetectionEventHasRequiredFields() throws Exception {
    ...
    DetectionEvent event =
        new HiddenFileDetectedEvent(
            "TestRule", Severity.LOW, Confidence.LOW, "Test description", java.util.List.of());
      ...
      if (!field.isSynthetic() && !Modifier.isStatic(field.getModifiers())) {
        ...
        fail(
            ruleClass.getSimpleName()
                + " should not have mutable instance state: "
                + field.getName());
      }
    }
  }
  ...
  void testDetectionContextIsImmutable() {
    assertTrue(
        Modifier.isFinal(DetectionContext.class.getModifiers()),
  ...
  void testDetectionResultIsImmutable() {
    assertTrue(
        Modifier.isFinal(DetectionResult.class.getModifiers()), "DetectionResult should be final");
  ...
  void testDetectionMetricsIsImmutable() {
    assertTrue(
        Modifier.isFinal(DetectionMetrics.class.getModifiers()),
  ...
  void testDetectionStateEnumCompleteness() {
    ...
    assertTrue(states.length >= 6, "Should have at least 6 states");
  ...
  void testDetectionExceptionExtendsException() {
    assertTrue(
        Exception.class.isAssignableFrom(DetectionException.class),
  ...
  void testDetectionEngineIsFinal() {
    assertTrue(
        Modifier.isFinal(DetectionEngine.class.getModifiers()), "DetectionEngine should be final");
  }
  ...
  void testDetectionEngineUsesProperSynchronization() throws Exception {
    ...
    assertTrue(
        Modifier.isSynchronized(startMethod.getModifiers()), "start() should be synchronized");
    assertTrue(Modifier.isSynchronized(stopMethod.getModifiers()), "stop() should be synchronized");
  }
