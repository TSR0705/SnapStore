 ...
 *
 * <ul>
 *   <li>Track recent alerts by suppression key
 *   <li>Enforce cooldown windows
 *   <li>Prevent duplicate alert flooding
 *   <li>Manage bounded suppression memory
 * </ul>
 *
 * <p>Suppression strategy:
 *
 * <ul>
 *   <li>Same suppression key within cooldown → suppress
 *   <li>Different suppression key → allow
 *   <li>Expired cooldown → allow
 * </ul>
 *
...
public final class SuppressionEngine {
  ...
  public boolean shouldSuppress(DetectionEvent detection) {
    ...
    if (lastAlertTime != null) {
      ...
      if (timeSinceLastAlert < cooldownMs) {
        log.debug(
            "Suppressing alert for key: {} ({}ms since last alert)",
            suppressionKey,
            timeSinceLastAlert);
        return true;
      }
  ...
  public int cleanupExpiredSuppressions() {
    ...
    int removed = 0;
    Iterator<Map.Entry<String, Long>> iterator = suppressionCache.entrySet().iterator();
   ...
   *
   * <ul>
   *   <li>Same rule + same severity + same path → suppress
   *   <li>Different severity → allow (escalation)
   *   <li>Different path → allow (different target)
   * </ul>
   */
