 ...
 *
 * <ul>
 *   <li>Generate correlation keys from detection events
 *   <li>Track active incidents by correlation key
 *   <li>Determine merge vs create decisions
 *   <li>Manage correlation memory with bounded growth
 * </ul>
 *
 * <p>Correlation strategy:
 *
 * <ul>
 *   <li>Same correlation key + within time window → merge
 *   <li>Different correlation key → new incident
 *   <li>Expired correlation → new incident
 * </ul>
 *
...
public final class CorrelationEngine {
  ...
  /**
   * Finds an existing incident that should be merged with this detection, or returns null if a new
  ...
  public void registerIncident(DetectionEvent detection, String incidentId) {
    ...
    CorrelationEntry entry =
        new CorrelationEntry(incidentId, correlationKey, System.currentTimeMillis());
   ...
   *
   * <ul>
   *   <li>MassDeletion + RapidModification + SuspiciousRename → ransomware pattern
   *   <li>Same rule + same path → file-specific pattern
   *   <li>Same rule + different paths → rule-specific pattern
   * </ul>
   */
