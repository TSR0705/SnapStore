 ...
 * <p>Design principles:
 * <ul>
 *   <li><b>Lazy loading</b> — views are loaded on first navigation, not at startup.</li>
 *   <li><b>Caching</b> — once loaded, a view's {@link Node} is cached and reused.
 *       FXML is never parsed more than once per view.</li>
 *   <li><b>Controller injection</b> — controllers receive the {@link AppContext}
 *       via a custom {@link FXMLLoader} controller factory, keeping them
 *       decoupled from static state.</li>
 *   <li><b>Content area ownership</b> — the manager holds a reference to the
 *       main content {@link StackPane} and swaps children on navigation.</li>
 * </ul>
 *
 * <p>Navigation is always performed on the JavaFX Application Thread.
...
public final class ViewManager {
    ...
    /**
     * Binds the content area pane that this manager will swap views into.
     * Must be called once after the main layout is loaded.
     *
     ...
     *
     * <p>If the view has been loaded before, the cached node is used.
     * Otherwise the FXML is loaded, the controller is initialized, and
     * the result is cached for future navigations.
     *
    ...
    /**
     * Returns the currently active view identifier, or {@code null} if
    ...
    /**
     * Pre-warms a view by loading it into the cache without displaying it.
     * Useful for views that should be instantly available on first navigation.
     *
    ...
    /**
     * Evicts a view from the cache, forcing a fresh load on next navigation.
     * Use sparingly — only when a view's state must be fully reset.
     *
    ...
    private CachedView loadView(ViewId viewId) {
        ...
        loader.setControllerFactory(controllerClass ->
                ControllerFactory.create(controllerClass, appContext)
        );

        try {
            Node node = loader.load();
            Object controller = loader.getController();
            log.info("View loaded: {} → controller: {}",
                    viewId, controller != null ? controller.getClass().getSimpleName() : "none");
