 ...
 *
 * <ul>
 *   <li>Event-by-event replay traversal
 *   <li>Replay windows with bounded loading
 *   <li>Replay checkpoints for navigation
 *   <li>Deterministic ordering using (timestamp, sequence_number)
 * </ul>
 *
...
public final class ReplayNavigationService {
  ...
  public ReplayWindow replayTimeWindow(Instant startTime, Instant endTime, int windowSize)
    ...
    try {
      ...
      List<ForensicTimelineEntity> events =
          timelineRepository.findByTimeRange(startTime, endTime, pageRequest).content();

      List<TimelineEventSummary> summaries =
          events.stream().map(this::toTimelineEventSummary).toList();
      ...
      return new ReplayWindow(
          summaries, startTime, endTime, 0, windowSize, totalCount, summaries.size() < totalCount);
  ...
  public ReplayWindow replayIncidentTimeline(String incidentId) throws InvestigationException {
    ...
    try {
      ...
      Instant endTime =
          summaries.isEmpty() ? Instant.now() : summaries.get(summaries.size() - 1).getTimestamp();

      return new ReplayWindow(
          summaries, startTime, endTime, 0, summaries.size(), summaries.size(), false);
  ...
  public ReplayWindow replayFromCheckpoint(ReplayCursor cursor) throws InvestigationException {
    ...
    try {
      ...
      if (cursor.direction() == ReplayDirection.FORWARD) {
        events =
            timelineRepository.findForwardFromCheckpoint(
                cursor.incidentId(),
                cursor.timestamp(),
        ...
        events =
            timelineRepository.findBackwardFromCheckpoint(
                cursor.incidentId(),
                cursor.timestamp(),
                cursor.sequenceNumber(),
                cursor.windowSize());
      ...
      Instant startTime =
          summaries.isEmpty() ? cursor.timestamp() : summaries.get(0).getTimestamp();
      Instant endTime =
          summaries.isEmpty()
              ? cursor.timestamp()
              : summaries.get(summaries.size() - 1).getTimestamp();
      ...
      throw new InvestigationException(
  ...
  public ReplayWindow findNextCheckpoint(Instant afterTimestamp, int windowSize)
    ...
    try {
      ...
      PageRequest pageRequest = new PageRequest(0, windowSize);
      List<ForensicTimelineEntity> events =
          timelineRepository.findByTimeRange(afterTimestamp, endTime, pageRequest).content();
