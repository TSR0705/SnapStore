    ...
    private static class IncidentListCell extends ListCell<IncidentSummary> {
        ...
        protected void updateItem(IncidentSummary incident, boolean empty) {
            ...
            if (empty || incident == null) {
                ...
                String icon = "ℹ️";
                String badgeStyle = "-fx-background-color: #1a2f26; -fx-text-fill: #4ade80; -fx-border-color: #22c55e;";
                switch (incident.getSeverity()) {
                    case "CRITICAL", "HIGH" -> {
                        icon = "🚨";
                        badgeStyle = "-fx-background-color: #3b1c1c; -fx-text-fill: #ff5a6a; -fx-border-color: #ff3333;";
                    }
                    case "MEDIUM" -> {
                        icon = "⚠️";
                        badgeStyle = "-fx-background-color: #3b2c1c; -fx-text-fill: #ffe066; -fx-border-color: #ff9933;";
                    }
                }
                
                Label lblBadge = new Label(icon + " " + incident.getSeverity());
                lblBadge.setStyle(badgeStyle + " -fx-font-weight: bold; -fx-font-size: 10px; -fx-padding: 2 6 2 6; -fx-background-radius: 4; -fx-border-radius: 4; -fx-border-width: 1;");
                ...
                lblTitle.setStyle("-fx-font-weight: bold; -fx-font-size: 13px; -fx-text-fill: -color-text;");
                
                String timeString = timeFormatter.format(incident.getCreatedAt());
                Label lblDetails = new Label(String.format("Detected at %s | 📊 %d Evidence Items", timeString, incident.getEvidenceCount()));
