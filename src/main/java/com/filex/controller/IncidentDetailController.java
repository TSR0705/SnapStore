 ...
 *
 * <ul>
 *   <li>Display incident summary and metadata
 *   <li>Provide entry points to evidence and replay
 *   <li>Show related incidents
 *   <li>Handle async incident loading
 * </ul>
 *
 * <p>Design principles:
 *
 * <ul>
 *   <li><b>Progressive disclosure</b> — summary first, details on demand
...
public final class IncidentDetailController {
    ...
    workspaceService.findIncidentByIdAsync(
        ...
        result ->
            result.ifPresentOrElse(
                this::displayIncident, () -> showError("Incident not found: " + incidentId)),
  ...
  private void navigateToEvidence() {
    ...
    workspaceController.loadDetailView(
        com.filex.ui.ViewId.EVIDENCE_EXPLORATION,
        controller -> {
  ...
  private void navigateToReplay() {
    ...
    workspaceController.loadDetailView(
        com.filex.ui.ViewId.TIMELINE_REPLAY,
        controller -> {
  ...
  private void navigateToRelated() {
    ...
    com.filex.investigation.InvestigationCriteria criteria =
        com.filex.investigation.InvestigationCriteria.builder()
  ...
  private static class EvidenceListCell extends ListCell<EvidenceSummary> {
    ...
    protected void updateItem(EvidenceSummary evidence, boolean empty) {
      ...
      if (empty || evidence == null) {
        ...
        setText(String.format("%s - %s", evidence.getRuleName(), evidence.getFilePath()));
      }
    }
  }
}
