 ...
 *
 * <p>Redesigned to present a high-contrast real-time Threat surveillance dashboard with live
 * telemetry updates, monitoring targets, active rules list, dynamic host target adding,
 * asynchronous Threat Simulation Studio, and a scrolling live Security Operations Terminal log
 * feed.
 */
public final class OverviewController {
  ...
  private java.util.function.Consumer<com.filex.detection.MassDeletionDetectedEvent>
      massDeletionSub;
  private java.util.function.Consumer<com.filex.detection.RapidModificationDetectedEvent>
      rapidModSub;
  ...
  private java.util.function.Consumer<com.filex.detection.SensitiveDirectoryAccessDetectedEvent>
      sensitiveSub;
            ...
            new KeyFrame(
                ...
                event -> {
                  ...
                  if (lblStatus.getScene() == null) {
                    ...
                    log.debug(
                        "OverviewController refresh timeline stopped dynamically as view detached.");
                    return;
                  }
                  updateTelemetry();
                }));
  ...
  public void onAddPath() {
    try {
      ...
      if (selectedDirectory != null) {
        ...
        appendLog(
            "[SYSTEM] Successfully registered new host path under active recursive surveillance: "
                + path.toAbsolutePath());
        ...
        () -> {
          try {
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] 💥 STARTING virtual Ransomware attack simulation in: "
                            + sandbox.toAbsolutePath()));

            // Create decoy files
            for (int i = 1; i <= 6; i++) {
              Path file = sandbox.resolve("decoy_ransom_" + i + ".txt");
              java.nio.file.Files.writeString(
                  file, "FileX safe decoy data block " + System.currentTimeMillis());
            }
            ...
            for (int i = 1; i <= 6; i++) {
              Path file = sandbox.resolve("decoy_ransom_" + i + ".txt");
              java.nio.file.Files.writeString(
                  file, "MUTATED AND LOCKED ENCRYPTION payload - " + System.currentTimeMillis());
              Thread.sleep(15);
            }
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] ✅ Ransomware attack pattern payload deployed. Evaluating detection correlation..."));
          } catch (Exception e) {
        ...
        () -> {
          try {
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] 🗑️ STARTING virtual Mass Deletion attack simulation in: "
                            + sandbox.toAbsolutePath()));
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] ✅ Mass Deletion pattern deployed. Evaluating detection correlation..."));
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        });
        ...
        () -> {
          try {
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] 🔄 STARTING Extension Renaming lock simulation in: "
                            + sandbox.toAbsolutePath()));
            ...
            java.nio.file.Files.move(
                srcFile, dstFile, java.nio.file.StandardCopyOption.REPLACE_EXISTING);

            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] ✅ Suspicious Extension Rename deployed. Evaluating detection correlation..."));
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        });
        ...
        () -> {
          try {
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] 🕵️ STARTING Stealthy Hidden Payload simulation in: "
                            + sandbox.toAbsolutePath()));
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] ✅ Stealthy Hidden File created. Evaluating detection correlation..."));
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        });
        ...
        () -> {
          try {
            ...
            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] 🔑 STARTING Sensitive Configuration Tamper simulation in: "
                            + sandbox.toAbsolutePath()));
            ...
            java.nio.file.Files.writeString(
                configFile, "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQC8... mock");

            Platform.runLater(
                () ->
                    appendLog(
                        "[SIMULATION] ✅ Sensitive Directory Activity pattern deployed. Evaluating detection correlation..."));
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        });
  ...
  private Path getSandboxPath() throws Exception {
    ...
    if (monitoredRoots.isEmpty()) {
      throw new IllegalStateException(
          "No target directories registered. Please register at least one target path first using '+ Add Target'!");
    }
    ...
    new Thread(
            () -> {
              try {
                ...
                Platform.runLater(
                    () -> appendLog("[ERROR] " + name + " failed: " + e.getMessage()));
              }
            })
        .start();
  }
  ...
  private void handleRawFileEvent(com.filex.engine.MonitoringEvent event, String op) {
    Platform.runLater(
        () -> {
          String pathStr =
              (event.path() != null) ? event.path().toAbsolutePath().normalize().toString() : "n/a";
  ...
  private void handleDetectionEvent(com.filex.detection.DetectionEvent event) {
    Platform.runLater(
        () -> {
          appendLog(
              String.format(
                  "[ALERT] ⚠️  RULE MATCHED: %s - %s [Severity: %s]",
  ...
  private void handleIncidentEvent(com.filex.alert.IncidentCreatedEvent event) {
    Platform.runLater(
        () -> {
          ...
          appendLog(
              String.format(
                  "[INCIDENT] 🚨 ALARM TRIGGERED: %s (ID: %s) [Severity: %s]",
  ...
  private void cleanupSubscriptions() {
    try {
      ...
      bus.unsubscribe(
          com.filex.detection.SensitiveDirectoryAccessDetectedEvent.class, sensitiveSub);
  ...
  private void updateTelemetry() {
    try {
      ...
      lblStatus
          .getStyleClass()
          .removeAll("kpi-value-active", "kpi-value-danger", "kpi-value-warning");
