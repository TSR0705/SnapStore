 ...
 *
 * <p>Triggers when files are created with suspicious extensions commonly associated with ransomware
 * encryption.
 *
 * <p>Suspicious extensions include: .encrypted, .locked, .crypted, .crypto, .crypt, .enc, .locky,
 * .cerber, etc.
 */
public final class SuspiciousExtensionRenameRule implements DetectionRule {
  ...
  private static final Set<String> SUSPICIOUS_EXTENSIONS =
      Set.of(
          ".encrypted",
          ".locked",
          ".crypted",
  ...
  public DetectionResult evaluate(DetectionContext context) {
    ...
    for (String suspiciousExt : SUSPICIOUS_EXTENSIONS) {
      if (filename.endsWith(suspiciousExt)) {
        log.warn("Suspicious file extension detected: {} (extension: {})", path, suspiciousExt);
