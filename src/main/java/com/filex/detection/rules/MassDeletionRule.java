 ...
 *
 * <p>Triggers when a large number of files are deleted within
 * a short time window, which may indicate ransomware or malicious
 * cleanup activity.
 *
 * <p>Thresholds:
 * <ul>
 *   <li>50+ deletions in 10 seconds → HIGH severity</li>
 *   <li>20-49 deletions in 10 seconds → MEDIUM severity</li>
 * </ul>
 */
public final class MassDeletionRule implements DetectionRule {
    ...
    public DetectionResult evaluate(DetectionContext context) {
        ...
        List<RawFileDeletedEvent> recentDeletions = context.getEventsWithinWindow(EVALUATION_WINDOW_MS)
                .stream()
        ...
        if (deletionCount >= HIGH_SEVERITY_THRESHOLD) {
            log.warn("Mass deletion detected: {} files deleted in {}ms",
                    deletionCount, EVALUATION_WINDOW_MS);
            ...
            return DetectionResult.detected(
                    Severity.HIGH,
                    Confidence.HIGH,
                    String.format("Mass deletion detected: %d files deleted in %d seconds",
                            deletionCount, EVALUATION_WINDOW_MS / 1000),
                    contextData
            );
        } else if (deletionCount >= MEDIUM_SEVERITY_THRESHOLD) {
            log.info("Elevated deletion activity detected: {} files deleted in {}ms",
                    deletionCount, EVALUATION_WINDOW_MS);
            ...
            return DetectionResult.detected(
                    Severity.MEDIUM,
                    Confidence.MEDIUM,
                    String.format("Elevated deletion activity: %d files deleted in %d seconds",
                            deletionCount, EVALUATION_WINDOW_MS / 1000),
                    contextData
            );
        }
