 ...
 *
 * <p>Triggers when files are modified at an unusually high rate,
 * which may indicate encryption activity or automated tampering.
 *
 * <p>Thresholds:
 * <ul>
 *   <li>100+ modifications in 10 seconds → HIGH severity</li>
 *   <li>50-99 modifications in 10 seconds → MEDIUM severity</li>
 * </ul>
 */
public final class RapidModificationRule implements DetectionRule {
    ...
    public DetectionResult evaluate(DetectionContext context) {
        ...
        List<RawFileModifiedEvent> recentModifications = context.getEventsWithinWindow(EVALUATION_WINDOW_MS)
                .stream()
        ...
        if (modificationCount >= HIGH_SEVERITY_THRESHOLD) {
            log.warn("Rapid modification detected: {} files modified in {}ms",
                    modificationCount, EVALUATION_WINDOW_MS);
            ...
            return DetectionResult.detected(
                    Severity.HIGH,
                    Confidence.MEDIUM,
                    String.format("Rapid modification detected: %d files modified in %d seconds",
                            modificationCount, EVALUATION_WINDOW_MS / 1000),
                    contextData
            );
        } else if (modificationCount >= MEDIUM_SEVERITY_THRESHOLD) {
            log.info("Elevated modification activity detected: {} files modified in {}ms",
                    modificationCount, EVALUATION_WINDOW_MS);
            ...
            return DetectionResult.detected(
                    Severity.MEDIUM,
                    Confidence.MEDIUM,
                    String.format("Elevated modification activity: %d files modified in %d seconds",
                            modificationCount, EVALUATION_WINDOW_MS / 1000),
                    contextData
            );
        }
