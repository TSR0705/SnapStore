 ...
 *
 * <ul>
 *   <li>Current monitoring event being evaluated
 *   <li>Recent event history for temporal correlation
 *   <li>Time window boundaries
 * </ul>
 *
...
public final class DetectionContext {
  ...
  public DetectionContext(
      MonitoringEvent currentEvent, List<MonitoringEvent> recentEvents, long windowMs) {
  ...
  /**
   * Returns recent monitoring events within the time window. List is immutable and ordered by
   * occurrence time (oldest first).
   */
  ...
  public <T extends MonitoringEvent> int countEventsOfType(Class<T> eventType) {
    return (int) recentEvents.stream().filter(eventType::isInstance).count();
  }

  /**
   * Returns events that occurred within the specified time window relative to the current event.
   */
