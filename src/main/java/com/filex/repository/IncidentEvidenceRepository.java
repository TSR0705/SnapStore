 ...
 *
 * <p>Manages immutable evidence records that link detections to incidents. Evidence records are
 * append-only and never modified, preserving forensic integrity.
 *
...
public final class IncidentEvidenceRepository {
    ...
    try (PreparedStatement pstmt =
      ...
      try (ResultSet rs = pstmt.getGeneratedKeys()) {
        if (rs.next()) {
          ...
          log.debug(
              "Inserted evidence: id={}, evidenceId={}, incidentId={}",
              id,
  ...
  public Optional<IncidentEvidenceEntity> findByDetectionId(String detectionId)
      throws SQLException {
  ...
  public Page<IncidentEvidenceEntity> findByRuleName(String ruleName, PageRequest pageRequest)
      throws SQLException {
    List<IncidentEvidenceEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM incident_evidence WHERE rule_name = ? ORDER BY detected_at DESC LIMIT ? OFFSET ?";
  ...
  public Page<IncidentEvidenceEntity> findByFilePath(String filePath, PageRequest pageRequest)
      throws SQLException {
    List<IncidentEvidenceEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM incident_evidence WHERE file_path = ? ORDER BY detected_at DESC LIMIT ? OFFSET ?";
  ...
  public List<IncidentEvidenceEntity> findByCorrelationId(String correlationId)
      throws SQLException {
    List<IncidentEvidenceEntity> evidence = new ArrayList<>();
    String sql =
        "SELECT * FROM incident_evidence WHERE correlation_id = ? ORDER BY detected_at ASC";
