 ...
 *
 * <p>Manages incident records with full CRUD support. Incidents are mutable (status and severity
 * can escalate), but all changes are tracked via timestamps.
 *
 * <p>All SQL is encapsulated here with PreparedStatement for safety. No SQL logic exists outside
 * this repository.
 *
...
public final class IncidentRepository {
  ...
  public List<IncidentEntity> findActive() throws SQLException {
    List<IncidentEntity> incidents = new ArrayList<>();
    String sql =
        "SELECT * FROM incidents WHERE status IN ('OPEN', 'INVESTIGATING') ORDER BY severity DESC, created_at DESC";
  ...
  public Page<IncidentEntity> findBySeverity(String severity, PageRequest pageRequest)
      throws SQLException {
    List<IncidentEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM incidents WHERE severity = ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
  ...
  public Page<IncidentEntity> findByTimeRange(
      Instant startTime, Instant endTime, PageRequest pageRequest) throws SQLException {
    List<IncidentEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM incidents WHERE created_at BETWEEN ? AND ? ORDER BY created_at DESC LIMIT ? OFFSET ?";
  ...
  /**
   * Deletes incidents older than the specified retention period. Only deletes resolved or dismissed
   * incidents.
   *
  ...
  public int deleteOlderThan(Instant olderThan) throws SQLException {
    String sql =
        "DELETE FROM incidents WHERE created_at < ? AND status IN ('RESOLVED', 'DISMISSED')";
