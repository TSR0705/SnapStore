 ...
 *
 * <p>Manages immutable timeline records for chronological reconstruction of security events.
 * Timeline records are ordered by timestamp and sequence number for deterministic replay.
 *
 * <p>All SQL is encapsulated here with PreparedStatement for safety.
 *
 * <p>Thread-safety: Repository instances are NOT thread-safe. Callers must ensure proper
 * synchronization or use separate instances per thread.
 */
public final class ForensicTimelineRepository {
    ...
    try (PreparedStatement pstmt =
      ...
      try (ResultSet rs = pstmt.getGeneratedKeys()) {
        if (rs.next()) {
          ...
          log.debug(
              "Inserted timeline record: id={}, timelineId={}, eventType={}",
              id,
  ...
  public List<ForensicTimelineEntity> findByIncidentId(String incidentId) throws SQLException {
    ...
    String sql =
        "SELECT * FROM forensic_timeline WHERE incident_id = ? ORDER BY timestamp ASC, sequence_number ASC";
  ...
  public List<ForensicTimelineEntity> findForwardFromCheckpoint(
      String incidentId, Instant timestamp, long sequenceNumber, int limit) throws SQLException {
    List<ForensicTimelineEntity> timeline = new ArrayList<>();
    String sql =
        """
  ...
  public List<ForensicTimelineEntity> findBackwardFromCheckpoint(
      String incidentId, Instant timestamp, long sequenceNumber, int limit) throws SQLException {
    List<ForensicTimelineEntity> timeline = new ArrayList<>();
    String sql =
        """
    ...
    try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
      ...
      try (ResultSet rs = pstmt.executeQuery()) {
        while (rs.next()) {
          // For backward fetch we get results in DESC order, but we want them displayed
          // chronologically
  ...
  public Page<ForensicTimelineEntity> findByEventType(String eventType, PageRequest pageRequest)
      throws SQLException {
    List<ForensicTimelineEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM forensic_timeline WHERE event_type = ? ORDER BY timestamp ASC, sequence_number ASC LIMIT ? OFFSET ?";
  ...
  public Page<ForensicTimelineEntity> findBySeverity(String severity, PageRequest pageRequest)
      throws SQLException {
    List<ForensicTimelineEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM forensic_timeline WHERE severity = ? ORDER BY timestamp ASC, sequence_number ASC LIMIT ? OFFSET ?";
  ...
  public List<ForensicTimelineEntity> findByCorrelationId(String correlationId)
      throws SQLException {
    List<ForensicTimelineEntity> timeline = new ArrayList<>();
    String sql =
        "SELECT * FROM forensic_timeline WHERE correlation_id = ? ORDER BY timestamp ASC, sequence_number ASC";
  ...
  public Page<ForensicTimelineEntity> findByTimeRange(
      Instant startTime, Instant endTime, PageRequest pageRequest) throws SQLException {
    List<ForensicTimelineEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM forensic_timeline WHERE timestamp BETWEEN ? AND ? ORDER BY timestamp ASC, sequence_number ASC LIMIT ? OFFSET ?";
  ...
  /**
   * Finds all timeline records, paginated and ordered chronologically (ascending for forward
   * replay).
   */
  public Page<ForensicTimelineEntity> findAll(PageRequest pageRequest) throws SQLException {
    List<ForensicTimelineEntity> content = new ArrayList<>();
    String sql =
        "SELECT * FROM forensic_timeline ORDER BY timestamp ASC, sequence_number ASC LIMIT ? OFFSET ?";
  ...
  /**
   * Gets the next sequence number for a given timestamp. Used to ensure deterministic ordering for
   * events with the same timestamp.
   */
  public long getNextSequenceNumber(Instant timestamp) throws SQLException {
    String sql =
        "SELECT COALESCE(MAX(sequence_number), 0) + 1 FROM forensic_timeline WHERE timestamp = ?";
