 ...
 *
 * <p>Represents the current focus area of the operator's investigation. Used for:
 *
 * <ul>
 *   <li>Context-aware navigation
 *   <li>Panel visibility control
 *   <li>Workflow state tracking
 *   <li>Session restoration
 * </ul>
 *
...
public enum NavigationContext {
  ...
  /** Operator is viewing the incident list (triage view). No specific incident selected. */
  INCIDENT_LIST,

  /** Operator is viewing a specific incident's details. Incident ID is set in workspace state. */
  INCIDENT_DETAIL,

  /**
   * Operator is exploring evidence chain for an incident. Both incident ID and evidence ID are set.
   */
  EVIDENCE_EXPLORATION,

  /**
   * Operator is replaying timeline events for an incident. Incident ID and replay checkpoint are
   * set.
   */
  TIMELINE_REPLAY,

  /** Operator is analyzing correlated incidents. Correlation ID is set in workspace state. */
  CORRELATION_ANALYSIS;
