 ...
 *
 * <ul>
 *   <li>Recursive directory watching
 *   <li>Dynamic subdirectory registration
 *   <li>Event normalization and deduplication
 *   <li>Lifecycle-safe startup/shutdown
 *   <li>Publishing monitoring events to EventBus
 * </ul>
 *
 * <p>Thread ownership:
 *
 * <ul>
 *   <li>MonitoringEngine owns watch loop thread
 *   <li>Watch loop remains lightweight (no DB writes, no heavy processing)
 *   <li>Events published async to EventBus for downstream processing
 * </ul>
 *
 * <p>The monitoring engine does NOT:
 *
 * <ul>
 *   <li>Perform detection logic
 *   <li>Write directly to database
 *   <li>Touch UI directly
 *   <li>Compute threat scores
 * </ul>
 */
public final class MonitoringEngine {
  ...
  public synchronized void start(List<Path> paths) throws MonitoringException {
    log.info(
        TruthMarkers.TRUTH,
        "component=MonitoringEngine event=engine_starting roots="
            + paths.stream().map(p -> p.toString().replaceAll("[\\s\\p{Cntrl}]", "_")).toList());
    ...
    try {
      ...
      log.info(
          TruthMarkers.TRUTH,
          "component=MonitoringEngine event=watch_roots_configured roots="
              + validRoots.stream()
      ...
      log.info(
          "Monitoring engine started successfully. Watching {} directories", watchKeyToPath.size());
      ...
      log.error(
          TruthMarkers.TRUTH,
          "component=MonitoringEngine event=registration_failed path="
              + paths.stream().map(p -> p.toString().replaceAll("[\\s\\p{Cntrl}]", "_")).toList()
              + " err="
              + (e.getMessage() == null
                  ? "null"
  ...
  private void registerRecursive(Path root) throws IOException {
    log.info(
        TruthMarkers.TRUTH,
        "component=MonitoringEngine event=recursive_registration_started root="
    ...
    log.info(
        TruthMarkers.TRUTH,
        "component=MonitoringEngine event=recursive_registration_completed root="
            + root.toString().replaceAll("[\\s\\p{Cntrl}]", "_")
            + " dirs="
            + regCount[0]);
  }
  ...
  /**
   * Main watch loop - runs on dedicated thread. Remains lightweight: only detects events and
   * publishes them.
   */
