 ...
 *
 * <p>The {@code AppContext} owns all core infrastructure components and provides them to the rest
 * of the application via explicit getters. This is NOT a service locator — dependencies are passed
 * explicitly through constructors, not pulled from a global static.
 *
 * <p>Lifecycle:
 *
 * <ol>
 *   <li>Constructed by {@link Bootstrap} with core infrastructure
 *   <li>ViewManager is set via {@link #setViewManager(ViewManager)} during bootstrap
 *   <li>WorkspaceService is set via {@link #setWorkspaceService(WorkspaceService)} during bootstrap
 *   <li>Passed to controllers via dependency injection
 *   <li>Held by {@link FileXApplication} until shutdown
 * </ol>
 *
...
public final class AppContext {
  ...
  /**
   * Package-private constructor — only {@link Bootstrap} should create instances. ViewManager and
   * WorkspaceService are set separately.
   */
  AppContext(
      AppConfig config,
    ...
    this.monitoringEngine =
        Objects.requireNonNull(monitoringEngine, "monitoringEngine must not be null");
    this.detectionEngine =
        Objects.requireNonNull(detectionEngine, "detectionEngine must not be null");
    this.alertEngine = Objects.requireNonNull(alertEngine, "alertEngine must not be null");
    this.incidentPersistenceService =
        Objects.requireNonNull(
            incidentPersistenceService, "incidentPersistenceService must not be null");
    this.incidentPersistenceSubscriber =
        Objects.requireNonNull(
            incidentPersistenceSubscriber, "incidentPersistenceSubscriber must not be null");
    this.runtimeManager =
        new com.filex.runtime.RuntimeManager(
            config,
            databaseManager,
            eventBus,
            monitoringEngine,
  ...
  /**
   * Sets the ViewManager. Called once by {@link Bootstrap} after AppContext creation. This breaks
   * the circular dependency between AppContext and ViewManager.
   *
  ...
  /**
   * Sets the WorkspaceService. Called once by {@link Bootstrap} after AppContext creation. This
   * breaks the circular dependency between AppContext and WorkspaceService.
   *
  ...
  void setWorkspaceService(WorkspaceService workspaceService) {
    ...
    this.workspaceService =
        Objects.requireNonNull(workspaceService, "workspaceService must not be null");
  }
  ...
  public ViewManager viewManager() {
    if (viewManager == null) {
      throw new IllegalStateException(
          "ViewManager not yet initialized. Called too early in bootstrap.");
    }
    return viewManager;
  }
  ...
  public WorkspaceService workspaceService() {
    if (workspaceService == null) {
      throw new IllegalStateException(
          "WorkspaceService not yet initialized. Called too early in bootstrap.");
    }
    return workspaceService;
  }

  /** Returns a human-readable summary of the context state. Useful for startup logging. */
