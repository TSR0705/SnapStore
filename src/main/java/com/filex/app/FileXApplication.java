 ...
 * <p>Lifecycle:
 * <ol>
 *   <li>{@link #init()} — runs on JavaFX launcher thread before UI starts.
 *       Executes the full {@link Bootstrap} sequence.</li>
 *   <li>{@link #start(Stage)} — runs on JavaFX Application Thread.
 *       Loads the main layout FXML and displays the primary stage.</li>
 *   <li>{@link #stop()} — runs on JavaFX Application Thread during shutdown.
 *       Publishes shutdown event and releases all resources.</li>
 * </ol>
 *
...
public final class FileXApplication extends Application {
    ...
    /**
     * JavaFX lifecycle: initialization phase.
     * Runs BEFORE the JavaFX Application Thread starts.
     *
     * <p>We perform the full bootstrap here so that any failures occur
     * before the UI is shown. This prevents showing a broken window.
     */
    @Override
    ...
    /**
     * JavaFX lifecycle: start phase.
     * Runs on the JavaFX Application Thread.
     *
     * <p>Loads the main layout FXML, configures the primary stage, and
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        if (fxmlUrl == null) {
            throw new IllegalStateException("MainLayout.fxml not found at: " + ViewId.MAIN_LAYOUT.fxmlPath());
        }

        FXMLLoader loader = new FXMLLoader(fxmlUrl);
        loader.setControllerFactory(controllerClass ->
                createController(controllerClass, appContext)
        );
    ...
    /**
     * JavaFX lifecycle: stop phase.
     * Runs on the JavaFX Application Thread during shutdown.
     *
     * <p>Publishes the shutdown event and releases all resources in
    ...
    /**
     * Controller factory for the main layout.
     * Delegates to the same factory logic used by ViewManager.
     */
