 ...
 * </ul>
 *
 * <p>All persistence operations are transaction-safe. Failures trigger rollback to prevent partial
 * writes and forensic corruption.
 *
 * <p>Thread-safety: This service is thread-safe. Multiple threads can call persistence methods
 * concurrently.
 */
public final class IncidentPersistenceService {
   ...
   * <ol>
   *   <li>Incident record
   *   <li>Evidence record linking the detection
   *   <li>Timeline record for incident creation
   * </ol>
   *
   ...
   */
  public void persistNewIncident(Incident incident, DetectionEvent detection)
    ...
    try {
      transactionTemplate.executeVoid(
          conn -> {
            // 1. Persist incident
            ...
            try {
              ...
              log.debug(
                  ...
                  incident.getIncidentId(),
                  System.currentTimeMillis() - incidentStartMs);
            ...
            ForensicTimelineEntity timelineEntity =
                createIncidentCreatedTimeline(incident, detection);
            try {
              ...
              log.debug(
                  ...
                  timelineEntity.getTimelineId(),
                  timelineEntity.getSequenceNumber());
            } catch (RuntimeException | SQLException ex) {
              log.error(
                  TruthMarkers.TRUTH,
            ...
            log.info(
                "Persisted new incident: incidentId={}, detectionId={}",
                incident.getIncidentId(),
                detection.eventId());
          });
      ...
      throw new PersistenceException(
          "Failed to persist new incident: " + incident.getIncidentId(), e);
    }
  }
   ...
   *
   * <ol>
   *   <li>Updated incident record
   *   <li>New evidence record (if detection provided)
   *   <li>Timeline record for the update
   * </ol>
   *
   ...
   * @throws PersistenceException if persistence fails
   */
  public void persistIncidentUpdate(
      Incident incident, DetectionEvent detection, String updateReason)
      throws PersistenceException {
    Objects.requireNonNull(incident, "incident must not be null");
    ...
    try {
      transactionTemplate.executeVoid(
          conn -> {
            ...
            // 3. Persist timeline record
            ForensicTimelineEntity timelineEntity =
                createIncidentUpdatedTimeline(incident, detection, updateReason);
            try {
              timelineRepository.insert(timelineEntity);
              log.debug(
                  TruthMarkers.TRUTH,
            ...
            log.debug(
                "Persisted incident update: incidentId={}, reason={}",
                incident.getIncidentId(),
                updateReason);
          });
      ...
      throw new PersistenceException(
          "Failed to persist incident update: " + incident.getIncidentId(), e);
    }
  }
   ...
   * @throws PersistenceException if recovery fails
   */
  public List<IncidentEvidenceEntity> recoverIncidentEvidence(String incidentId)
      throws PersistenceException {
    try {
  ...
  private IncidentEntity mapToEntity(Incident incident) {
    ...
    String metadata =
        incident.getEvidenceSummary().isEmpty() ? null : incident.getEvidenceSummary().toString();
  ...
  private IncidentEvidenceEntity createEvidenceEntity(Incident incident, DetectionEvent detection) {
    ...
    String filePath =
        detection.affectedPaths().isEmpty() ? null : detection.affectedPaths().get(0).toString();
  ...
  private ForensicTimelineEntity createIncidentCreatedTimeline(
      Incident incident, DetectionEvent detection) {
  ...
  private ForensicTimelineEntity createIncidentUpdatedTimeline(
      Incident incident, DetectionEvent detection, String updateReason) {
    String timelineId = UUID.randomUUID().toString();
