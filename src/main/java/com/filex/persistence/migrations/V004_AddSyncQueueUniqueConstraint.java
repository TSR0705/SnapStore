 ...
 *
 * <p>Prevents duplicate pending sync operations for the same entity. Uses a partial unique index
 * that only applies to pending operations, allowing multiple completed/failed entries for the same
 * entity.
 *
 * <p>Constraint: (entity_type, entity_id, operation) must be unique when status = 'pending'.
 */
