 ...
 *
 * <p>Indexes are carefully chosen to balance read performance with write overhead. Focus is on
 * common query patterns:
 *
 * <ul>
 *   <li>Time-range queries on events and alerts
 *   <li>Filtering by event type and severity
 *   <li>Suspicious activity queries
 *   <li>File path lookups
 *   <li>Sync queue status filtering
 * </ul>
 */
public final class V002_CreateIndexes implements Migration {
  ...
  public void migrate(Connection connection) throws SQLException {
    try (Statement stmt = connection.createStatement()) {
      ...
      stmt.execute(
          "CREATE INDEX IF NOT EXISTS idx_file_events_suspicious ON file_events(suspicious, timestamp DESC);");
      stmt.execute(
      ...
      stmt.execute(
          "CREATE INDEX IF NOT EXISTS idx_fingerprints_file_path ON file_fingerprints(file_path);");
      stmt.execute(
          "CREATE INDEX IF NOT EXISTS idx_fingerprints_file_hash ON file_fingerprints(file_hash);");
      stmt.execute(
          "CREATE INDEX IF NOT EXISTS idx_fingerprints_last_seen ON file_fingerprints(last_seen DESC);");
